...
def optimize_sharpe():
    ...
    # Run once to get stock_price_df, listed_options
    on_trade_fns_buy_and_hold = [buy_stock_on_day_0(amount=100)]
    trades_buy_and_hold, portfolio_df_buy_and_hold, stock_price_df, listed_options = (
        run_backtest(config, on_trade_fns_buy_and_hold)
    )

    def objective(trial: Trial) -> float:
        # Define the search space for the parameters
        dte_range_start = trial.suggest_int(
            "dte_range_start", 7, 70, step=7
        )  # Range in days
        delta = trial.suggest_float("delta", -0.9, -0.1, step=0.05)
        # expiration_days = trial.suggest_int("expiration_days", 1, 720)
        ...
        def buy_put_rollover() -> OnTradeFn:
                ...
                return rollover_to(
                    {
                        ...
                        # "dte_gt": dte_gt,
                        # "dte_lt": dte_lt,
                        "dte_range": (dte_range_start, dte_range_start + 7),
                        "sort_by_delta_near": delta,
                    },
                    buy_amount=1,
                )
            ...
            condition_action_pairs = [
                (if_expiration_within_days(3), rollover_to_delta(delta)),
            ]
            default_action = buy_option(
                {
                    "option_type": "put",
        ...
        on_trade_fns_protective_put = [
            buy_stock_on_day_0(amount=100),
            buy_put_rollover(),
        ]

        (trades_protective_put, portfolio_df_protective_put, _, _) = run_backtest(
            config, on_trade_fns_protective_put, stock_price_df, listed_options
        )

        metrics_protective_put = calculate_metrics(portfolio_df_protective_put, 0)
        sharpe = metrics_protective_put["Sharpe"].iloc[0]
    ...
    study.optimize(
        objective, n_trials=100, catch=[RolloverOptionNotFoundError], n_jobs=4
    )
